// ============================================================
// ADR Web Validator - Main Application
// ============================================================

import { validateADR } from './validator.js';
...
const storage = createStorage('adr-validator-history');
...
const scoreConsequences = document.getElementById('score-consequences');
...
function updateScoreDisplay(result) {
  ...
  scoreContext.textContent = result.context.score;
  scoreDecision.textContent = result.decision.score;
  scoreConsequences.textContent = result.consequences.score;
  scoreStatus.textContent = result.status.score;

  // Apply colors to dimension scores
  scoreContext.className = getScoreColor(result.context.score, result.context.maxScore);
  scoreDecision.className = getScoreColor(result.decision.score, result.decision.maxScore);
  scoreConsequences.className = getScoreColor(result.consequences.score, result.consequences.maxScore);
  scoreStatus.className = getScoreColor(result.status.score, result.status.maxScore);

  // Update progress bars
  const totalPercent = (result.totalScore / 100) * 100;
  const contextPercent = (result.context.score / result.context.maxScore) * 100;
  const decisionPercent = (result.decision.score / result.decision.maxScore) * 100;
  const consequencesPercent = (result.consequences.score / result.consequences.maxScore) * 100;
  const statusPercent = (result.status.score / result.status.maxScore) * 100;

  const scoreBar = document.getElementById('score-bar');
  const contextBar = document.getElementById('score-context-bar');
  const decisionBar = document.getElementById('score-decision-bar');
  const consequencesBar = document.getElementById('score-consequences-bar');
  const statusBar = document.getElementById('score-status-bar');

  if (scoreBar) scoreBar.style.width = `${totalPercent}%`;
  if (contextBar) contextBar.style.width = `${contextPercent}%`;
  if (decisionBar) decisionBar.style.width = `${decisionPercent}%`;
  if (consequencesBar) consequencesBar.style.width = `${consequencesPercent}%`;
  if (statusBar) statusBar.style.width = `${statusPercent}%`;
}
...
function runValidation() {
  ...
  currentResult = validateADR(content);
...
function showAbout() {
  ...
  modal.innerHTML = `
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md mx-4 shadow-xl">
      <h2 class="text-xl font-bold mb-4 dark:text-white">ADR Validator</h2>
      <p class="text-gray-600 dark:text-gray-300 mb-4">
        A client-side tool for validating Architecture Decision Records against best practices.
      </p>
      <p class="text-gray-600 dark:text-gray-300 mb-4">
        <strong>Scoring Dimensions:</strong><br>
        • Context (25 pts)<br>
        • Decision (25 pts)<br>
        • Consequences (25 pts)<br>
        • Status (25 pts)
      </p>
