...
export async function renderProjectsList() {
  ...
  container.innerHTML = `
    ...
    return `<div class="flex-1 h-1.5 rounded ${colorClass}"></div>`;
                            ...
                            <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 line-clamp-1">
...
export function renderNewProjectForm(existingProject = null) {
  ...
  // Template selector click handlers (only for new projects)
  if (!isEditing) {
      ...
      btn.addEventListener('click', () => {
        ...
        if (template) {
          ...
          if (template.status) {
            ...
            if (statusEl) statusEl.value = template.status;
          }
        }
      });
    });
